    ...
    evaluate() {
        ...
        links.forEach(link => {
            ...
            if (src && src.id && tgt && tgt.id) {
                if (adj.has(src.id)) {
                    adj.get(src.id).push({ link, neighborId: tgt.id, portFrom: src.port, portTo: tgt.port });
                }
                if (adj.has(tgt.id)) {
                    adj.get(tgt.id).push({ link, neighborId: src.id, portFrom: tgt.port, portTo: src.port });
                }
            }
        });
        ...
        elements.forEach(el => {
            ...
            if (catalog.isEnergizedSource && st !== 'DEAD' && st !== 'OPEN') {
                ...
                if (isOnline) {
                    liveQueue.push({ id: el.id, voltage: sldData.voltage || 154, color: sldData.color || '#7A3E9D' });
                    liveNodes.add(el.id);
                    nodeVoltages.set(el.id, { voltage: sldData.voltage || 154, color: sldData.color || '#7A3E9D' });
                }
            }
        });

        while (liveQueue.length > 0) {
            const { id: currentId, voltage: currentVoltage, color: currentColor } = liveQueue.shift();
            ...
            for (const edge of neighbors) {
                ...
                if (!conducting) {
                    ...
                    linkStatus.set(edge.link.id, { state: 'LIVE', voltageColor: currentColor });
                    continue;
                }
                ...
                liveLinks.add(edge.link.id);
                linkStatus.set(edge.link.id, { state: 'LIVE', voltageColor: nextColor });

                if (!liveNodes.has(edge.neighborId)) {
                    liveNodes.add(edge.neighborId);
                    nodeVoltages.set(edge.neighborId, { voltage: nextVoltage, color: nextColor });
                    liveQueue.push({ id: edge.neighborId, voltage: nextVoltage, color: nextColor });
                }
            }
        }
        ...
        links.forEach(link => {
            if (groundedLinks.has(link.id)) {
                ...
                const info = linkStatus.get(link.id) || { state: 'LIVE', voltageColor: '#377DFF' };
